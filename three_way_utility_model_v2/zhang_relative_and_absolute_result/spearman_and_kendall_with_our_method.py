...
real = []
# 读取排序结果数据
with open('..\\our_model_result\\winequality_red_ranking_result.csv') as f:
...
# 读取比较方法排序数据
with open('winequality_red_zhang_relative_and_absolute_ranking_result.csv') as f:
    read_csv = csv.reader(f)
    for row in read_csv:
...
ranking_result_by_object_index = [1 for i in range(len(real[1]) + 1)]
for i in range(len(real[1])):
    ranking_result_by_object_index[real[1][i]] = i + 1
real[1] = ranking_result_by_object_index[1:]
...
with open('spearman_and_kendall_rusult.csv', 'a', newline='') as f:
    ...
    for i in range(len(table)):
        # print(real)
        s = math.fabs(float(format(pd.Series(table[i][1:]).corr(pd.Series(real[1]), method='spearman'), '.4f')))
        k = math.fabs(float(format(pd.Series(table[i][1:]).corr(pd.Series(real[1]), method='kendall'), '.4f')))
