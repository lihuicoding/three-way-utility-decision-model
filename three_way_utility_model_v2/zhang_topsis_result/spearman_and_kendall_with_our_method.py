...
# 读取比较方法排序数据
with open('winequality_white_zhang_topsis_ranking_result.csv') as f:
    read_csv = csv.reader(f)
    for row in read_csv:
...
with open('spearman_and_kendall_rusult.csv', 'a', newline='') as f:
    ...
    for i in range(len(table)):
        # print(real)
        s = float(format(pd.Series(table[i][1:]).corr(pd.Series(real[0]), method='spearman'), '.4f'))
        k = float(format(pd.Series(table[i][1:]).corr(pd.Series(real[0]), method='kendall'), '.4f'))
